...
版本：v1.2
状态：执行中（B0 已于 2026-09-14 由用户决策取消；B1–B4 已完成；B5-1 因 13.3 源样本移出 `PPTX/` 阻塞，B5-2 已被 14.1 v5 交付覆盖；剩余可执行切片为 B5-1 复跑与可选 B6）
更新时间：2026-09-16
...
| B5-1 13.3 代入链批次 | ⏸ 阻塞 | 13.3 源样本已被移出 `PPTX/`（v1–v17 报告仍在 reports/）；需用户恢复样本或指定新链头后重跑 |
